...
public class CursorUnit : MonoBehaviour
{
    ...
    {
        bool moved = false;
        int index = transform.GetComponentInParent<MenuUnitController>().GetButtonList().IndexOf(transform.GetComponentInParent<MenuUnitController>().GetSelectedButton());

        if (transform.GetComponentInParent<MenuUnitController>().GetButtonList().Count > 1)
        ...
        if (index != -1)
        {
            ...
            {
                transform.GetComponentInParent<MenuUnitController>().SelectButton(
                    transform.GetComponentInParent<MenuUnitController>().GetButtonList().Count - 1);
                moved = true;
            }
            else
            {
                transform.GetComponentInParent<MenuUnitController>().SelectButton(index - 1);
                moved = true;
            }
        }
    ...
    public void MoveDown()
    {
        ...
        if (index != -1)
        {
            if (index == transform.GetComponentInParent<MenuUnitController>().GetButtonList().Count - 1) // el botó és el de baix de tot
            {
                transform.GetComponentInParent<MenuUnitController>().SelectButton(0);
                moved = true;
            }
            else
            {
    ...
    void Reposition()
    {
        transform.position = transform.GetComponentInParent<MenuUnitController>().GetSelectedButton().transform.position + new Vector3(-1, 0, 0);
        AdjustOffset();
    }
