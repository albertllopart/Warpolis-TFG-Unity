...
public class UnitAerial : MonoBehaviour
{
    ...
    public IEnumerator OnAI()
    {
        ...
        while (target != null)
        {
            ...
            {
                Debug.Log("UnitAerial::OnAI - Found Target: " + target.name + "in Position: " + target.transform.position);
                yield break;
            }
    ...
    {
        Debug.Log("UnitAerial::RoadToEnemy");
        ...
        foreach (Vector2Int intersection in intersections)
        {
            if (GetComponent<Unit>().CheckTileForAlly(new Vector3(intersection.x, intersection.y)) == null)
            {
                ...
                Debug.Log("UnitAerial::RoadToEnemy - Found Closest Available Tile to Goal at Position: " + nextStep);
                break;
            }
        }
